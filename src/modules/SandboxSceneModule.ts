...
import { GameplaySceneModule, type GameplaySceneConfig } from './GameplaySceneModule'
import type { SceneDescriptor } from '@base/scene-builder'

// ─── Hard-geometry definitions ─────────────────────────────────────────────────

interface PlatformDef {
  label: string; color: number; cx: number
  rampStartZ: number; rampEndZ: number; topEndZ: number
  width: number; height: number
}
interface ObstacleDef {
  label: string; color: number; cx: number; cz: number
  width: number; depth: number; height: number
}
interface PoolDef {
  minX: number; maxX: number
  minZ: number; maxZ: number   // minZ = north (deep), maxZ = south (shallow entry)
  surfaceY: number             // floor Y at maxZ (south, shallow)
  depthY: number               // floor Y at minZ (north, deepest)
}

// Parallel platforms — side by side along X, all ramp bases at z=+20 (south),
// ascending northward (decreasing Z).  Ramp length = height × 2 → ≈ 26.6° slope.
...
class SandboxSampler implements TerrainSurfaceSampler {
  sample(x: number, z: number): number {
    if (x >= POOL.minX && x <= POOL.maxX && z >= POOL.minZ && z <= POOL.maxZ) {
      // south (maxZ=25) = shallow (surfaceY=0), north (minZ=-25) = deep (depthY=-25)
...
function obstacleSample(x: number, z: number, o: ObstacleDef): number {
  if (x >= o.cx - o.width / 2 && x <= o.cx + o.width / 2 &&
      z >= o.cz - o.depth / 2 && z <= o.cz + o.depth / 2) return o.height
  return 0
}

// ─── Module ────────────────────────────────────────────────────────────────────

export class SandboxSceneModule extends GameplaySceneModule {
  private sandboxMeshes: THREE.Object3D[] = []

  constructor(options: Partial<GameplaySceneConfig> & { descriptor?: SceneDescriptor } = {}) {
    super({
      ...options,
      debugJumpArc: true,
  ...
  private addPlatform(scene: THREE.Scene, p: PlatformDef): void {
    const mat = new THREE.MeshStandardMaterial({ color: p.color, roughness: 0.55, metalness: 0.05, transparent: true, opacity: 0.88 })
    const wire = new THREE.LineBasicMaterial({ color: p.color })

    // Wedge ramp — rampStartZ (south) > rampEndZ (north); rotation.y=π so local +Z → world −Z.
    ...
    // Tier disc marker — south of ramp base (approach side).
  ...
  private addPool(scene: THREE.Scene): void {
    const w  = POOL.maxX - POOL.minX
    const l  = POOL.maxZ - POOL.minZ
    const d  = Math.abs(POOL.depthY)
    ...
    // North wall (z = minZ = −25) — deep end; south (maxZ=25) is open entry.
    addWall(new THREE.BoxGeometry(w, d, 0.4), mx, -d / 2, POOL.minZ - 0.2)

    // Sloped floor: south (maxZ) = shallow (Y=0), north (minZ) = deep (Y=−25).
    const slopeAngle = Math.atan2(d, l)
    ...
    // Water surface plane — pool footprint only (seaLevel is suppressed in descriptor).
    ...
    const waterMesh = new THREE.Mesh(waterGeo, new THREE.MeshStandardMaterial({
      ...
      roughness: 0.1, metalness: 0.2, side: THREE.FrontSide, depthWrite: false,
    }))
    ...
    const rimPts: number[] = []
    for (const [cx, cz] of corners) rimPts.push(cx, 0.05, cz)
    const rimGeo = new THREE.BufferGeometry()
    rimGeo.setAttribute('position', new THREE.BufferAttribute(new Float32Array(rimPts), 3))
    scene.add(new THREE.Line(rimGeo, wireMat))

    // Depth ticks — south (maxZ) = shallow, north (minZ) = deep.
    for (let i = 0; i <= 5; i++) {
      const t  = i / 5
      const fz = POOL.maxZ - l * t
      const fy = POOL.surfaceY + (POOL.depthY - POOL.surfaceY) * t
      const tGeo = new THREE.BufferGeometry()
      tGeo.setAttribute('position', new THREE.BufferAttribute(
        new Float32Array([POOL.minX, fy + 0.04, fz, POOL.maxX, fy + 0.04, fz]), 3,
      ))
      scene.add(new THREE.Line(tGeo, new THREE.LineBasicMaterial({ color: new THREE.Color().setHSL(0.55, 0.8, 0.72 - t * 0.4) })))
    }
  }
}
...
function makeWedge(width: number, length: number, height: number): THREE.BufferGeometry {
  ...
  const idx = new Uint16Array([
    0, 2, 1,   3, 4, 5,   // triangle faces
    0, 1, 4,   0, 4, 3,   // bottom
    1, 2, 5,   1, 5, 4,   // slope face
    2, 0, 3,   2, 3, 5,   // back vertical face
  ])
  const geo = new THREE.BufferGeometry()
